    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside :class="sidebarOpen ? 'w-64' : 'w-20'" class="bg-white  flex flex-col transition-all duration-300">
            ...
            <nav class="flex-1 overflow-y-auto p-4 space-y-1">
                <!-- Dashboard -->
                <a href="{{ route('petugas.dashboard') }}" class="flex items-center space-x-3 px-4 py-3 {{ request()->routeIs('petugas.dashboard') ? 'text-cyan-600 bg-cyan-50' : 'text-gray-700 hover:bg-gray-50' }} rounded-lg">
                ...
                <div x-data="{ open: {{ request()->routeIs(['peminjaman.*']) ? 'true' : 'false' }} }">
                    <button @click="open = !open" class="flex items-center justify-between w-full px-4 py-3 text-gray-700 hover:bg-gray-50 rounded-lg">
                    ...
                    <div x-show="open && sidebarOpen" x-collapse class="ml-8 mt-2 space-y-2">
                        <a href="{{ route('peminjaman.index') }}"
                           class="block px-4 py-2 text-sm {{ request()->routeIs('peminjaman.index') ? 'text-cyan-600 font-medium' : 'text-gray-600 hover:text-cyan-600' }}">
                            Daftar Peminjaman
                        </a>
                        <a href="{{ route('peminjaman.riwayat') }}"
                           class="block px-4 py-2 text-sm {{ request()->routeIs('peminjaman.riwayat') ? 'text-cyan-600 font-medium' : 'text-gray-600 hover:text-cyan-600' }}">
                ...
                <a href="{{ route('denda.index') }}"
                   class="flex items-center space-x-3 px-4 py-3 rounded-lg {{ request()->routeIs('denda.*') ? 'bg-cyan-50 text-cyan-600' : 'text-gray-700 hover:bg-gray-50' }}">
